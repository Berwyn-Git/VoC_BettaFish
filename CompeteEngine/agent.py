...
from .tools import TavilyNewsAgency, TavilyResponse, multilingual_sentiment_analyzer, SENTIMENT_ANALYZER_AVAILABLE
...
class DeepSearchAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        # 初始化情感分析器（通用工具）
        if SENTIMENT_ANALYZER_AVAILABLE and multilingual_sentiment_analyzer:
            self.sentiment_analyzer = multilingual_sentiment_analyzer
        else:
            self.sentiment_analyzer = None
            logger.warning("情感分析工具不可用，将跳过情感分析功能")
    ...
    def _initialize_llm(self) -> LLMClient:
        """初始化LLM客户端"""
        # 优先使用新配置，向后兼容旧配置
        api_key = self.config.COMPETE_ENGINE_API_KEY or self.config.QUERY_ENGINE_API_KEY
        model_name = self.config.COMPETE_ENGINE_MODEL_NAME or self.config.QUERY_ENGINE_MODEL_NAME
        base_url = self.config.COMPETE_ENGINE_BASE_URL or self.config.QUERY_ENGINE_BASE_URL
        return LLMClient(
            api_key=api_key,
    ...
    def analyze_sentiment_only(self, texts: List[str]) -> Dict[str, Any]:
        """
        独立的情感分析工具（通用工具）
        ...
        try:
            ...
            # 执行批量分析
            texts_list = list(texts) if isinstance(texts, list) else [texts]
